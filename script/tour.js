...
function init() {
    ...
    camera.update = function(time) {
        if (typeof track !== "undefined" && global.track.enabled) {
            if (global.track.counter < 1) {
                global.track.counter += global.track.speed;
            } else {
                global.track.counter = 0;
            }
            if (global.track.lookAt <= 1) {
                global.track.lookAt += global.track.speed;
            } else {
                global.track.lookAt = 0;
            }
            if (global.track.counter > 1) {
                global.track.counter = 1
            }
            if (global.track.lookAt > 1) {
                global.track.lookAt = 1
            }
            var position_l = track.getPointAt(global.track.counter);
            var position_r = track_r.getPointAt(global.track.counter);

            var lookAt_position_l = track.getPointAt(global.track.lookAt);
            var lookAt_position_r = track_r.getPointAt(global.track.lookAt);
            ...
            if (!global.mode.vr) {
                tangent = track.getTangentAt(global.track.lookAt).normalize();
        ...
        if (typeof wagons !== "undefined" && global.rad.enabled) {
            if (this.position.x == 0 && this.position.y == 0 && this.position.z == 0) {
                this.position.set(0, 0, 0);
                this.lookAt(new THREE.Vector3(1, 0, 0));
            }

            if (!global.mode.vr && controls.name !== 'orbit') {
        ...
        if (typeof monster !== "undefined" && global.monster.enabled) {
            if (!global.mode.vr && controls.name !== 'orbit') {
                controls = orbit_controls(controls);
        ...
        if (global.rad.enabled) {

        }

        if (global.explore.enabled) {
    ...
    loader.load(global.model.track, function(geometry) {
        var spline_geometry = [];
    ...
    ///rad_ground.json
    loader.load(global.model.rad_ground, function(geometry, material) {
    ...
    loader.load(global.model.rad, function(geometry, material) {
    ...
    loader.load(global.model.rad_wagon, function(geometry) {
    ...
    loader.load(global.model.monster, function(geometry, material) {
        material.forEach(function(mat, index) {
            mat.shading = THREE.FlatShading;
        });
    ...
    loader.load(global.model.monster_ground, function(geometry, material) {
    ...
    loader.load(global.model.park, function(geometry, material) {
    ...
    loader.load(global.model.cloud, function(geometry) {
...
action_rollercoaster.addEventListener('click', function(event) {
    camera.position.set(0, 0, 0);
    global.track.enabled = true;
    global.monster.enabled = false;
    global.rad.enabled = false;
...
action_monster.addEventListener('click', function(event) {
    camera.position.set(0, 0, 0);
    global.track.enabled = false;
    global.monster.enabled = true;
    global.rad.enabled = false;
...
function render(time) {
    ...
    if (global.mode.orientation) {
        effect.render(scene, camera);

    }
    // update skinning
    if (global.mode.desktop || !global.mode.orientation) {
        camera.update(time);
    } else {
